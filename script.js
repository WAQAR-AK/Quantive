...
generateQuestionsBtn.addEventListener('click', async function() {
    ...
    try {
        ...
        // On Glitch, if your Flask app is the main server, this relative path should work.
        ...
        if (generatedQuestions && generatedQuestions.length > 0) {
            generatedQuestions.forEach(suggestion => {
                // Ensure suggestion has a type, default to 'text' if missing
                if (!suggestion.type) {
                    console.warn("AI suggestion missing type, defaulting to 'text':", suggestion);
                    suggestion.type = 'text';
                }
                 // Ensure options is an array for choice types
